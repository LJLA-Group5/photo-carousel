...
const edgeFromUserListsStream = fs.createWriteStream(path.join(__dirname, '/../data/arangoEdgeFromUserListsData.csv'));
...
const genEdgeFromUserLists = () => {
  ...
  const _from = `userLists/${genNumBtwn(1, 200000)}`;
  const _to = `listings/${genNumBtwn(1, 10000000)}`;
  const edgeFromUserListsEntry = `${_from}, ${_to}\n`;
  const streamOkay = edgeFromUserListsStream.write(edgeFromUserListsEntry);
